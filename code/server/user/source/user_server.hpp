...
public:
  ...
  virtual void UserLogin(google::protobuf::RpcController *controller,
    ...
    if (!user || user->password() != password) {
      LOG_ERROR("{} - 用户名或密码错误 - {} ", request->request_id(), nickname);
  ...
  virtual void PhoneRegister(google::protobuf::RpcController *controller,
    ...
    if (vcode != code) {
      LOG_ERROR("{} - 手机验证码错误 - {} - {}", request->request_id(), code_id,
                code);
  ...
  virtual void PhoneLogin(google::protobuf::RpcController *controller,
    ...
    auto user = _mysql_user->select_by_phone(phone);
    if (!user) {
      LOG_ERROR("{} - 该手机号未注册过用户 - {}", request->request_id(), phone);
  ...
  virtual void GetMultiUserInfo(google::protobuf::RpcController *controller,
    ...
    auto channel = _mm_channels->choose(_file_service_name);
    if (!channel) {
      LOG_ERROR("{} - 未找到文件管理子服务节点 - {}", request->request_id(),
                _file_service_name);
      return err_response(request->request_id(), "未找到文件管理子服务节点");
    }
    FileService_Stub stub(channel.get());
  ...
  virtual void SetUserAvatar(google::protobuf::RpcController *controller,
    ...
    if (ret == false) {
      LOG_ERROR("{} - 更新Mysql数据库用户头像ID失败", request->request_id());
      return err_response(request->request_id(),
                          "更新Mysql数据库用户头像ID失败");
    }
    ...
    if (ret == false) {
      LOG_ERROR("{} - 更新ES搜索引擎用户头像ID失败", request->request_id());
      return err_response(request->request_id(), "ES搜索引擎用户头像ID失败");
    }
    response->set_success(true);
    response->set_request_id(request->request_id());
  }
  virtual void SetUserNickname(google::protobuf::RpcController *controller,
    ...
    if (ret == false) {
      LOG_ERROR("{} - 更新Mysql数据库用户昵称失败", request->request_id());
      return err_response(request->request_id(), "更新Mysql数据库用户昵称失败");
    }
    ret = _es_user->appendData(user->user_id(), user->phone(), user->nickname(),
                               user->description(), user->avatar_id());
    if (ret == false) {
      LOG_ERROR("{} - 更新ES搜索引擎用户昵称失败", request->request_id());
      return err_response(request->request_id(), "更新ES搜索引擎用户昵称失败");
    }
    response->set_success(true);
    response->set_request_id(request->request_id());
  }
  ...
  SetUserDescription(google::protobuf::RpcController *controller,
    ...
    bool ret = _mysql_user->update(user);
    if (ret == false) {
      LOG_ERROR("{} - 更新Mysql数据库用户签名失败", request->request_id());
      return err_response(request->request_id(), "更新Mysql数据库用户签名失败");
    }
    ret = _es_user->appendData(user->user_id(), user->phone(), user->nickname(),
                               user->description(), user->avatar_id());
    if (ret == false) {
      LOG_ERROR("{} - 更新ES搜索引擎用户签名失败", request->request_id());
      return err_response(request->request_id(), "更新ES搜索引擎用户签名失败");
    }
    response->set_success(true);
    response->set_request_id(request->request_id());
  }
  ...
  SetUserPhoneNumber(google::protobuf::RpcController *controller,
    ...
    if (ret == false) {
      LOG_ERROR("{} - 手机号码格式错误 - {}", request->request_id(), new_phone);
      return err_response(request->request_id(), "用户名或密码错误");
    }
    auto vcode = _redis_code->code(code_id);
    if (vcode != code) {
    ...
    ret = _mysql_user->update(user);
    if (ret == false) {
      LOG_ERROR("{} - 更新Mysql数据库用户手机号失败", request->request_id());
      return err_response(request->request_id(),
                          "更新Mysql数据库用户手机号失败");
    }
    ret = _es_user->appendData(user->user_id(), user->phone(), user->nickname(),
                               user->description(), user->avatar_id());
    if (ret == false) {
      LOG_ERROR("{} - 更新ES搜索引擎用户手机号失败", request->request_id());
      return err_response(request->request_id(),
                          "更新ES搜索引擎用户手机号失败");
    }
    response->set_success(true);
...
public:
  ...
  void make_mysql_object(const std::string &user, const std::string &passwd,
                         ...
                         int &conn_pool_count) {
    _mysql_client = im_server::ODBFactory::create(user, passwd, host, db, cset,
                                                  port, conn_pool_count);
  }
