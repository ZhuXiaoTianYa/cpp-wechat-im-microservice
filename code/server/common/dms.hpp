...
public:
    ...
    bool send(const std::string &phone, const std::string &code) {
        ...
        for (auto &p : params) {
            ...
            char *v =
                curl_easy_escape(_curl, p.second.c_str(), p.second.size());
            query += k;
            query += "=";
            query += v;
            curl_free(k);
        ...
        char *encodedQuery =
            curl_easy_escape(_curl, query.c_str(), query.size());
        stringToSign += encodedQuery;
        curl_free(encodedQuery);

        std::string signature =
            hmacSha1Base64(_access_key_secret + "&", stringToSign);
        std::string postData = query + "&Signature=";
        char *sigEncoded =
            curl_easy_escape(_curl, signature.c_str(), signature.size());
        postData += sigEncoded;
...
private:
    ...
    static std::string hmacSha1Base64(const std::string &key,
