    ...
    bool create(const std::string &index_id = "default_index_id") {
        ...
        try {
            ...
            if (resp.status_code < 200 || resp.status_code >= 300) {
                LOG_ERROR("创建ES索引{}失败，响应状态码异常:{}, 响应消息{}",
                          _name, resp.status_code, resp.text);
                return false;
            }
...
public:
    EsInsert(std::shared_ptr<elasticlient::Client> &client,
             const std::string &name, const std::string &type = "_doc")
        : _name(name), _type(type), _client(client) {}
    template <class T> EsInsert &append(const std::string &key, const T &val) {
        _item[key] = val;
        return *this;
    }
    ...
    bool insert(const std::string &id) {
        ...
        try {
            ...
            if (resp.status_code < 200 || resp.status_code >= 300) {
                LOG_ERROR("新增数据{}失败，响应状态码异常:{}, 响应消息{}",
                          _name, resp.status_code, resp.text);
                return false;
            }
        } catch (std::exception &e) {
...
public:
    ...
    bool remove(const std::string &id) {
        try {
            ...
            if (resp.status_code < 200 || resp.status_code >= 300) {
                LOG_ERROR("删除数据{}失败，响应状态码异常:{}, 响应消息{}",
                          _name, resp.status_code, resp.text);
                return false;
            }
        } catch (std::exception &e) {
...
public:
    ...
    EsSearch &append_must_not_term(const std::string &key,
    ...
    EsSearch &append_must_match(const std::string &key,
                                const std::string &val) {
        Json::Value match;
        Json::Value field;
        field[key] = val;
    ...
    Json::Value search() {
        ...
        try {
            ...
            if (resp.status_code < 200 || resp.status_code >= 300) {
                LOG_ERROR("查询数据{}失败，响应状态码异常:{}, 响应消息{}",
                          _name, resp.status_code, resp.text);
                return Json::Value();
            }
