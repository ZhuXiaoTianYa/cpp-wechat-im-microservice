...
namespace im_server
{
    ...
    public:
        ...
        void declareComponents(const std::string &exchange, const std::string &queue, const std::string &routing_key = "routing_key", AMQP::ExchangeType exchange_type = AMQP::ExchangeType::direct)
        {
            _channel->declareExchange(exchange, AMQP::ExchangeType::direct).onError([](const char *message)
                                                                                    { LOG_ERROR("声明交换机失败：{}", message);
                                                                                    exit(0); })
                .onSuccess([exchange]()
                           { LOG_DEBUG("{} 交换机创建成功：", exchange); });
            _channel->declareQueue(queue).onError([](const char *message)
                                                  {LOG_ERROR("声明队列失败：{}", message);
                                                                                    exit(0); })
                .onSuccess([queue]()
                           { LOG_DEBUG("{} 队列创建成功：", queue); });

            _channel->bindQueue(exchange, queue, routing_key).onError([exchange, queue](const char *message)
        ...
        void consume(const std::string &queue, const MessageCallback &cb)
        {
            ...
            _channel->consume(queue, "consume-tag").onReceived([this, cb](const AMQP::Message &message, uint64_t deliveryTag, bool redelivered)
                ...
                .onError([queue](const char *message)
                         { LOG_ERROR("订阅{}消息失败: {}", queue, message); exit(0); });
        }
