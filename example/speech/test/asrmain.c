...
RETURN_CODE run_asr(struct asr_config *config, const char *token)
{
    ...
    struct json_object *json_obj = json_tokener_parse(result);
    if (json_obj != NULL)
    {
        // 1. 获取错误码
        struct json_object *err_no;
        json_object_object_get_ex(json_obj, "err_no", &err_no);
        int code = json_object_get_int(err_no);

        // 2. 获取错误信息
        struct json_object *err_msg;
        json_object_object_get_ex(json_obj, "err_msg", &err_msg);
        const char *msg = json_object_get_string(err_msg);

        // 3. 获取识别结果
        struct json_object *result_array;
        json_object_object_get_ex(json_obj, "result", &result_array);
        const char *text = json_object_get_string(json_object_array_get_idx(result_array, 0));

        // ============== 你可以随便用这些值了 ==============
        printf("\n===== 解析后的 JSON 对象内容 =====\n");
        printf("错误码 err_no: %d\n", code);
        printf("错误信息 err_msg: %s\n", msg);
        printf("识别结果: %s\n", text);
